...
Product decision **1B** (archive reopening, 2026-07-31) supersedes the archive portion of 1A. Durable page archives are implemented for Local and File bookmarks only. Pinboard, Sync, and Browser bookmarks remain metadata-only. The archive contract is sanitized readable HTML plus extracted text, with product screenshots stored as separate artifacts.
...
| Feature | Status |
|---------|--------|
| Full-text **content archiving** (save page body) | Implemented under 1B for Local/File |
| Full-text search over **archived** content | Implemented under 1B |
| **Reader / offline reading** mode | Implemented under 1B |
| **Screenshot** page archives (product) | Implemented under 1B ([IMPL-SCREENSHOT_MODE] remains demo capture only) |
...
- **Reopened and implemented under 1B:** The four archive features above are the only reopened path. They use the Local/File scope, privacy lifecycle, separate screenshot-artifact boundaries, backend-scoped Reader identity, structured Reader failures, and configured File-adapter validation recorded in the corresponding TIED REQ/ARCH/IMPL decisions.
