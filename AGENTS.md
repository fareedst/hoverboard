...
- **TIED data access (MCP-first)**
  ...
  - **YAML structural rules for detail files** (see `detail-files-schema.md` § Structural rules and common errors): **`detail_file` is index-only** — never include it inside a detail YAML under `requirements/`, `architecture-decisions/`, or `implementation-decisions/`. **Top-level key** must be the token; all fields nest under it. **`cross_references`**, **`traceability.*`**, and **`related_requirements.*` / `related_decisions.*`** values are always **lists** (use `[]` when empty). **Quote** scalars that contain `: ` (colon-space), `@`, `#`, `!`, or other YAML-special characters. MCP write tools help with quoting; they do not fix wrong shapes or forbidden fields — prefer MCP, and when editing files directly apply these rules and § 3.1.1 in `processes.md` (`[PROC-YAML_EDIT_LOOP]`).
