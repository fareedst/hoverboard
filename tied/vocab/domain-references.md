...
| 9 | [`side-panel.md`](side-panel.md) | Side panel tabs: This Page, By Tag, Tabs; Visit History / Browser Bookmarks pages |
...
- **Bookmarks overloaded** — Hoverboard-stored pins ([`bookmarks.md`](bookmarks.md)), **Browser Bookmarks (page)** (standalone Chrome tree UI, [`side-panel.md`](side-panel.md) / [REQ-SIDE_PANEL_BROWSER_BOOKMARKS](../requirements/REQ-SIDE_PANEL_BROWSER_BOOKMARKS.yaml); legacy token name, not a side-panel tab), **Local Bookmarks Index** ([`bookmarks-index.md`](bookmarks-index.md)), and **Browser storage (backend)** / Store B ([`storage-backends.md`](storage-backends.md) / [REQ-BROWSER_BOOKMARK_STORAGE](../requirements/REQ-BROWSER_BOOKMARK_STORAGE.yaml)) are distinct. **Browser Bookmark Import** copies FROM the Chrome tree INTO Local|File|Sync only (Browser excluded as Import-to target).
- **Usage vs Visit History** — Prefer **Visit History (page)** (`visit-history.html`); **Usage (panel)** is a demoted legacy synonym for the former side-panel tab ([`side-panel.md`](side-panel.md) / [REQ-BOOKMARK_USAGE_TRACKING](../requirements/REQ-BOOKMARK_USAGE_TRACKING.yaml)).
- **web protocol / tools toolbar / tool page shell** — Non-`http(s)` active tabs dismiss the **side panel** and open the **tools toolbar** badge popup ([REQ-NON_WEB_TOOLS_TOOLBAR](../requirements/REQ-NON_WEB_TOOLS_TOOLBAR.yaml)); standalone Index/Import/Options/Browser Bookmarks/Visit History share **tool page shell** + **tool-page-version**; see [`side-panel.md`](side-panel.md) and [`ui-surfaces.md`](ui-surfaces.md).
