...
- **TIED base path before writes:** In MCP sessions, call **`tied_config_get_base_path`** and confirm it points at the **`tied/` of the repository you are changing**. When using **`tied-cli.sh`**, set **`TIED_BASE_PATH`** to that same **`tied/`** (absolute). Wrong `TIED_BASE_PATH` mutates another project’s `tied/` silently (see `AGENTS.md` §2 TIED data access, CITDP [IMPL-TIED_FILES] [ARCH-TIED_STRUCTURE] [REQ-TIED_SETUP] (was RISK-010)). Fix with an absolute `TIED_BASE_PATH` in that repo’s **`.cursor/mcp.json`**, the shell environment for **`tied-cli.sh`**, or re-run `copy_files.sh` targeting that repo.
