...
# Add/delete tags to selected bookmarks; parseTagsInput, mergeTags, removeTags, selectionStillVisible; saveBookmark per row.
# Parse comma-separated input; trim and dedupe case-insensitive.
...
# Merge new tags with existing; case-insensitive dedupe.
...
# Remove given tags from existing list (case-insensitive).
...
# Return set of selected URLs that remain in filtered list.
...
# For each selected URL merge new tags and send saveBookmark; refresh and restore selection for still-visible.
...
# For each selected URL remove tags and send saveBookmark; refresh and restore selection for still-visible.
