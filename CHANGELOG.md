...
- **Extension icon click opens side panel in current window** ([REQ-ICON_CLICK_BEHAVIOR], [ARCH-ICON_CLICK_BEHAVIOR], [IMPL-ICON_CLICK_BEHAVIOR]) – Clicking the extension toolbar icon now opens the side panel in the **window that has focus** (the window where you clicked), instead of sometimes opening it in a different window. The icon click handler no longer uses the cached `_sidePanelWindowId` from startup; it always uses `chrome.tabs.query({ active: true, currentWindow: true }, callback)` at click time so the panel opens in the current window and that window is focused. When the current tab is a restricted page (e.g. `chrome://extensions`), the extension finds an alternative http(s) tab and opens the panel there, then focuses that window. Extension commands, context menu, and OPEN_SIDE_PANEL from the popup still use `_openSidePanelWithFallback()` (cached or cold-start). Unit tests: icon click always uses current-window query path; restricted-tab fallback; cache ignored for icon click. TIED: REQ satisfaction criterion and ARCH/IMPL essence_pseudocode updated.
...
- **Side panel Bookmark and Tags tree: demo GIFs** ([PROC-DEMO_RECORDING], [IMPL-DEMO_OVERLAY], [REQ-SIDE_PANEL_POPUP_EQUIVALENT], [REQ-SIDE_PANEL_TAGS_TREE], [IMPL-SIDE_PANEL_BOOKMARK], [IMPL-SIDE_PANEL_TAGS_TREE], [PROC-TIED_DEV_CYCLE]) – Animated demos for the **Bookmark** tab ([docs/demo-bookmark.gif](docs/demo-bookmark.gif)) and **Tags tree** tab ([docs/demo-tags-tree.gif](docs/demo-tags-tree.gif)), same look and feel as the Tabs demo (overlay, viewport, ffmpeg two-pass GIF). Scripts: `scripts/record-demo-bookmark.js`, `scripts/record-demo-tags-tree.js`. [PROC-DEMO_RECORDING] and [IMPL-DEMO_OVERLAY] updated in `tied/processes.md` and `tied/implementation-decisions/IMPL-DEMO_OVERLAY.yaml`.

- **Side panel Tabs tab: demo GIF (find and export)** ([PROC-DEMO_RECORDING], [REQ-SIDE_PANEL_BROWSER_TABS], [IMPL-SIDE_PANEL_BROWSER_TABS], [PROC-TIED_DEV_CYCLE]) – A brief animated demo at [docs/demo-tabs-export.gif](docs/demo-tabs-export.gif) shows opening the side panel, switching to the **Tabs** tab, and exporting visible tab records (Copy Records). Recorded via Playwright E2E and converted to GIF. Process [PROC-DEMO_RECORDING] documented in `tied/processes.md`; script `scripts/record-demo-tabs.js` and spec `tests/playwright/extension-demo-tabs.spec.js` for future demos.
