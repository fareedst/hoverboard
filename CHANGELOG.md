...
- **Current README media refresh** ([IMPL-SCREENSHOT_MODE], [IMPL-DEMO_OVERLAY], [PROC-DEMO_RECORDING]) – Refreshes side-panel and standalone Browser Bookmarks / Visit History captures, renames recording commands, and synchronizes seeded media evidence with the IMPL sidecars. CITDP: `tied/citdp/CITDP-REQ-NON_WEB_TOOLS_TOOLBAR-media-refresh_20260804.yaml`.
- **TIED validation traceability closeout** ([REQ-COMPOSITION_TEST_RECOGNITION], [REQ-MODULE_VALIDATION], [REQ-RELIABILITY], [IMPL-COMPOSITION_TEST_PATTERNS], [IMPL-BOOKMARK_ROUTER], [IMPL-MESSAGE_HANDLING]) – Added 13 UI-free composition suites, synchronized affected IMPL full blocks, and recorded CITDP/evidence boundaries; 184 Jest suites and 55 Chromium extension E2E tests passed, while the legacy Jest E2E script remains a zero-test command.
- **Local Bookmarks Index live Browser import and collapsible controls** ([REQ-LOCAL_BOOKMARKS_INDEX], [REQ-LOCAL_BOOKMARKS_INDEX_IMPORT], [REQ-BROWSER_BOOKMARK_IMPORT], [ARCH-LOCAL_BOOKMARKS_INDEX], [ARCH-BROWSER_BOOKMARK_IMPORT], [IMPL-LOCAL_BOOKMARKS_INDEX], [IMPL-LOCAL_BOOKMARKS_INDEX_IMPORT], [IMPL-BROWSER_BOOKMARK_IMPORT]) – Browser-tree import now lives in the Index with cleaned-URL collapse, target-scoped conflicts, folder/tag merging, and `source=browser` entry points; the fixed footer starts collapsed and preserves accessible one-group behavior. **Tests:** 1,689 unit tests passed with 1 skipped, 101 composition tests passed, and 5 extension Playwright tests passed; lint, typecheck, token/manifest, YAML, pseudo-code, and TIED consistency validation passed. CITDP: `tied/citdp/CITDP-REQ-LOCAL_BOOKMARKS_INDEX-FOOTER_COLLAPSE_20260803.yaml`, `tied/citdp/CITDP-REQ-BROWSER_BOOKMARK_IMPORT-index-integration_20260803.yaml`.
- **Side-panel Suggested Tag states and casing correction** ([REQ-SUGGESTED_TAGS_DEDUPLICATION], [REQ-SUGGESTED_TAGS_CASE_PRESERVATION], [REQ-SIDE_PANEL_POPUP_EQUIVALENT], [ARCH-SUGGESTED_TAGS], [ARCH-THIS_PAGE_TAG_SORT], [IMPL-SUGGESTED_TAGS], [IMPL-THIS_PAGE_TAG_SORT]) – This Page now renders absent, case-match, and case-mismatch suggestions with accessible Add/Remove/Replace actions, applies case conversion only to Suggested Tags, and persists corrections atomically while preserving unrelated tags. **Tests:** 1,681 Jest tests passed (1,580 unit, 101 composition; 1 skipped) and 2 This Page extension Playwright tests passed; source lint, typecheck, token/manifest, pseudo-code, and TIED consistency validation passed. CITDP: `tied/citdp/CITDP-REQ-SUGGESTED_TAGS_DEDUPLICATION_20260802.yaml`, `CITDP-REQ-SUGGESTED_TAGS_ADJUSTED_STATE_COMPARISON_20260803.yaml`, `CITDP-REQ-SUGGESTED_TAGS_CASE_CONVERSION_20260803.yaml`, `CITDP-REQ-SUGGESTED_TAGS_THREE_STATES_20260803.yaml`.
- **Read Later Browser metadata fallback** ([REQ-READ_LATER_BROWSER_FALLBACK], [ARCH-MOVE_BOOKMARK_UI], [IMPL-MOVE_BOOKMARK_UI]) – New Read Later saves selected as Browser resolve through the configured default metadata-capable backend, falling back to Local when the default is Browser; persisted updates and ordinary Save to routing remain unchanged. **Tests:** unit and composition coverage; no feature-specific E2E required. CITDP: `tied/citdp/CITDP-REQ-READ_LATER_BROWSER_FALLBACK_20260802.yaml`.
- **Enabled-by-default link health and Store counts** ([REQ-LINK_HEALTH], [ARCH-LINK_HEALTH], [IMPL-LINK_HEALTH], [REQ-LOCAL_BOOKMARKS_INDEX], [ARCH-LOCAL_BOOKMARKS_INDEX], [IMPL-LOCAL_BOOKMARKS_INDEX]) – Link health is enabled when its setting is absent while an explicit `false` remains an opt-out; configuration and page loading never dispatch network checks automatically. Local Bookmarks Index Stores (Local/File/Sync/Browser) start checked and show filtered / total provider-row counts before Store selection, including duplicate rows and Local fallback data. **Tests:** link-health/config/options/SW unit coverage; Store-count/filter/index unit coverage; updated Index Playwright assertions. CITDP: `tied/citdp/CITDP-REQ-HEALTH_STORE_COUNTS_20260802-closeout.yaml`.
- **Cross-resource retrieval and portable library packages** ([REQ-CROSS_RESOURCE_RETRIEVAL], [REQ-LIBRARY_PORTABILITY], [ARCH-CROSS_RESOURCE_RETRIEVAL], [ARCH-LIBRARY_PORTABILITY], [IMPL-CROSS_RESOURCE_RETRIEVAL], [IMPL-LIBRARY_PORTABILITY]) – Added the read-only All resources Local Bookmarks Index scope with canonical scope validation, isolated adapter states, source-aware actions, and control gating, plus deterministic checksum-verified non-secret library package export/import planning and compensation boundaries. **Focused tests:** 211 unit, 22 composition, and 4 extension Playwright tests passed. CITDP: `tied/citdp/CITDP-REQ-RETRIEVAL_PORTABILITY_20260801.yaml`.
- **Modular side panel runtime and production bundle tooling** ([REQ-SIDE_PANEL_POPUP_EQUIVALENT], [REQ-SIDE_PANEL_BUNDLE_SIZE], [ARCH-SIDE_PANEL_TABS], [ARCH-SIDE_PANEL_BUNDLE], [IMPL-SIDE_PANEL_TABS], [IMPL-SIDE_PANEL_BUNDLE]) – Split side-panel tab state, shell, controller, event bindings, and Bookmark-tab initialization into focused modules; production builds now minify all five esbuild entry points, and `npm run analyze:side-panel` reports unminified, minified, gzip, and contributor sizes. **Tests:** 1,615 Jest passed with 1 skipped across 164 suites; 14 side-panel composition tests; extension Playwright suite green. CITDP: `tied/citdp/CITDP-REQ-SIDE_PANEL_BUNDLE_SIZE.yaml`.
- **Backend-scoped archive and Reader identity** ([REQ-PAGE_ARCHIVE_STORAGE], [REQ-ARCHIVED_CONTENT_SEARCH], [REQ-OFFLINE_READER_MODE], [ARCH-PAGE_ARCHIVE_STORAGE], [ARCH-ARCHIVED_CONTENT_SEARCH], [ARCH-OFFLINE_READER_MODE], [IMPL-PAGE_ARCHIVE_STORAGE], [IMPL-ARCHIVED_CONTENT_SEARCH], [IMPL-OFFLINE_READER_MODE]) – Local and File archives sharing a URL retain distinct backend/archive identities through search, cleanup, and Offline Reader navigation; structured Reader failures preserve stable missing, unsupported-backend, storage-failure, and invalid-archive states. **Tests:** 1,598 Jest passed with 1 skipped across 158 suites, plus 8 archive extension Playwright E2E tests. CITDP: `tied/citdp/CITDP-REQ-PAGE_ARCHIVE_BACKEND_IDENTITY_20260801.yaml`.
- **Source presentation profile for archived pages** ([REQ-OFFLINE_READER_MODE], [REQ-PAGE_ARCHIVE_STORAGE], [ARCH-OFFLINE_READER_MODE], [ARCH-PAGE_ARCHIVE_STORAGE], [IMPL-OFFLINE_READER_MODE], [IMPL-PAGE_ARCHIVE_STORAGE]) – Capture stores bounded computed background, text, link, and color-scheme metadata separately from sanitized HTML; Offline Reader applies it to chrome and content only after WCAG AA validation, otherwise clearing source variables and using the Reader theme fallback without changing archive identity hashes. **Tests:** 1,588 Jest passed with 1 skipped, 12 archive composition tests, and 8 archive Playwright E2E tests. CITDP: `tied/citdp/CITDP-OFFLINE_READER_SOURCE_PRESENTATION_20260801.yaml`.
- **Selected-backend archive artifact status UI** ([REQ-PAGE_ARCHIVE_STATUS_UI], [ARCH-PAGE_ARCHIVE_STATUS_UI], [IMPL-PAGE_ARCHIVE_STATUS_UI]) – Popup and This Page now show independent readable archive and screenshot saved-state indicators for Local/File, clear stale URL/backend state, and gate Offline Reader on readable archive availability with an accessible unavailable-state explanation while keeping recapture controls enabled. **Tests:** 1581 Jest tests passed, 1 skipped, plus 8 archive extension Playwright E2E tests. CITDP: `tied/citdp/CITDP-REQ-PAGE_ARCHIVE_STATUS_UI.yaml`.
- **Local/File page archives, Offline Reader, archive search, and product screenshots** ([REQ-PAGE_ARCHIVE_STORAGE], [ARCH-PAGE_ARCHIVE_STORAGE], [IMPL-PAGE_ARCHIVE_STORAGE], [REQ-ARCHIVED_CONTENT_SEARCH], [REQ-OFFLINE_READER_MODE], [REQ-PAGE_SCREENSHOT_ARCHIVE]) – Explicit HTTP(S) capture stores sanitized readable archives and separate screenshot artifacts for Local/File bookmarks; archive-content scope lists/searches stored text with resolvable Reader targets, and Reader presents stored HTML/screenshots without live-page fetches. Selected-backend association preserves stubs/metadata, creates missing bookmarks, and exposes archive-retention, restoration, cleanup-failure, and bookmark-created diagnostics. **Tests:** 1562 passed, 1 skipped across 156 Jest suites, 84 composition integration tests, and 4 archive extension Playwright E2E tests. CITDP: `tied/citdp/CITDP-REQ-PAGE_ARCHIVE_FEATURES.yaml`, `tied/citdp/CITDP-REQ-PAGE_ARCHIVE_STORAGE.yaml`.

- **Fixed Local Bookmarks Index control panels** ([REQ-LOCAL_BOOKMARKS_INDEX], [ARCH-LOCAL_BOOKMARKS_INDEX], [IMPL-LOCAL_BOOKMARKS_INDEX]) – Added accessible fixed head/footer tabs with one visible group per region, viewport-safe sticky offsets, and preserved existing filter/action wiring. Tests: unit, static composition, and Playwright E2E coverage. CITDP: `tied/citdp/CITDP-REQ-LOCAL_BOOKMARKS_INDEX-fixed-control-panels.yaml`.

- **Non-web tools toolbar + standalone Browser Bookmarks / Visit History** ([REQ-NON_WEB_TOOLS_TOOLBAR], [ARCH-NON_WEB_TOOLS_TOOLBAR], [IMPL-NON_WEB_TOOLS_TOOLBAR], [REQ-SIDE_PANEL_BROWSER_BOOKMARKS], [REQ-BOOKMARK_USAGE_TRACKING], [IMPL-BOOKMARK_USAGE_TRACKING_UI]) – On non-`http(s)` active tabs, SW dismisses the side panel (`REQUEST_SIDE_PANEL_CLOSE`) and syncs the badge popup to `tools-toolbar.html` (Index, Import, Options, Browser Bookmarks, Visit History). Browser Bookmarks and Visit History are standalone pages (`browser-bookmarks.html`, `visit-history.html`) on shared **tool page shell** (`tool-page-shell.css`, `initToolPageVersion`); side panel tabs are This Page / By Tag / Tabs only. Shared `isWebProtocolUrl`. **Tests:** unit `web-protocol`, `non-web-tools-toolbar`, `side-panel-tabs`/`html`, `tool-page-version`, `usage-panel`, `browser-bookmarks-panel`; composition `non-web-tools-toolbar-composition`, `tools-toolbar-launchers-composition`, `standalone-tool-pages-composition`; Playwright `extension-usage-tracking.spec.js` (Visit History + Browser Bookmarks pages; toolbar HTML; removed side-panel tabs). Gesture-bound `action.openPopup`/`sidePanel.open` E2E deferred when fixture cannot invoke toolbar click. CITDP: `tied/citdp/CITDP-REQ-NON_WEB_TOOLS_TOOLBAR.yaml`.

- **Composition test recognition** ([REQ-COMPOSITION_TEST_RECOGNITION], [ARCH-COMPOSITION_TEST_PATTERNS], [IMPL-COMPOSITION_TEST_PATTERNS], [PROC-TEST_STRATEGY]) – Canonical pattern catalog in `tied/vocab/test-composition.md` (routed from `routing.md`); testability ladder (unit → composition → justified E2E); `npm run composition:plan` (`scripts/composition-test-plan.js` + pure `composition-test-plan-core.js`); generated-test template `tests/integration/_templates/composition-test.template.js`. **New composition suites:** popup message timeout×selection, createPopup scoped root, suggested-tag chip→addTag, openTagsTree→OPEN_SIDE_PANEL, missing message response, popup link-health hint. **Unit:** `composition-test-plan.test.js`. Seed inventory: `tied/working/composition-edge-classification.md`. CITDP: `tied/citdp/CITDP-REQ-COMPOSITION_TEST_RECOGNITION.yaml`. **No Playwright E2E** (unit/composition sufficient).

- **Link health opt-in + This Page hint** ([REQ-LINK_HEALTH], [IMPL-LINK_HEALTH]) – Options **Enable link health checks** (`linkHealthChecksEnabled`, enabled when absent and disabled only by explicit `false`); Index Check button gated; SW rejects `CHECK_LINK_HEALTH` when disabled; This Page/popup compact **Health:** hint from stored map when enabled. Helpers `isLinkHealthChecksEnabled`, `formatLinkHealthHint`, `applyLinkHealthHint`, `applyLinkHealthControlsGate`. **Tests:** `link-health.test.js`, `link-health-sw.test.js`, `options-link-health.test.js`; composition `bookmarks-table-link-health-composition`, `popup-link-health-hint-composition`. CITDP: `tied/citdp/CITDP-REQ-LINK_HEALTH-opt-in-hint.yaml`. **No Playwright E2E**.

- **Link health inhibit skip + fetch timeout** ([REQ-LINK_HEALTH], [IMPL-LINK_HEALTH]) – `CHECK_LINK_HEALTH` skips URLs matching `hoverboard_inhibit_urls` (record `unreachable`/`inhibited`, no fetch); each HEAD/GET uses AbortController (`LINK_HEALTH_FETCH_TIMEOUT_MS`, default 8s; abort → `unreachable`/`timeout`); status-only (no GET body buffer). Helpers `urlMatchesInhibitList`, `fetchWithLinkHealthTimeout`. **Tests:** `link-health.test.js`, `link-health-sw.test.js`. CITDP: `tied/citdp/CITDP-REQ-LINK_HEALTH-inhibit-timeout.yaml`.

- **Post-ship test coverage gaps (Index / snapshot / link-health / Local API)** ([REQ-LINK_HEALTH], [REQ-LIBRARY_SEARCH_ENTRY], [REQ-LOCAL_QUERY_API]) – Extracted Index orchestrators `prefillSearchFromQuery`, `runCheckLinkHealth`, `runRefreshApiSnapshot` (composition-testable; Index uses `filterBookmarksByHealth`). **Tests:** `bookmarks-table-library-search.test.js`; composition `bookmarks-table-link-health-composition`, `bookmarks-table-api-snapshot-composition`; `local-query-api-snapshot-write.integration.test.js` (real `_refreshApiSnapshot` → `sendNativeMessage`); SW edges in `link-health-sw.test.js` (batch cap 50, non-http filter, HEAD 501→GET, HEAD 404 no GET, `_getLinkHealthMap`); Go `TestHTTPContractViaServer` (`httptest.NewServer` + Bearer). CITDP: `tied/citdp/CITDP-REQ-BOOKMARK_MANAGER_COMPLETENESS-test-coverage-gaps.yaml`. **No Playwright E2E**. (Inhibit/timeout product behavior: see R1a entry above.)
...
- **Collections / collaboration docs** – `docs/COLLECTIONS_AND_COLLABORATION.md` (tags-as-collections; File-share ad hoc collaboration).

- **Bookmark Title and Notes on This Page / popup** ([REQ-BOOKMARK_NOTES_UI], [ARCH-BOOKMARK_NOTES_UI], [IMPL-BOOKMARK_NOTES_UI]) – Details section with Title (`description`) and Notes (`extended`); Save details / blur persist via `saveBookmark` with Save-to `preferredBackend`; Notes disabled with hint on Browser backend. Shared helpers `src/shared/bookmark-notes-ui.js`. Side panel Save-to includes Browser. **Tests:** unit `bookmark-notes-ui.test.js`; composition `bookmark-notes-ui.integration.test.js` (UIManager→`saveBookmarkDetails`). CITDP: `tied/citdp/CITDP-REQ-BOOKMARK_MANAGER_COMPLETENESS-notes-search-health-local-api.yaml`. **No Playwright E2E**.

- **Library Search entry + API snapshot** ([REQ-LIBRARY_SEARCH_ENTRY], [IMPL-LIBRARY_SEARCH_ENTRY], [REQ-LOCAL_QUERY_API]) – Capture UI **Search Bookmarks** opens Index with `?q=`; Index/Options **Refresh API snapshot** writes `aggregate-snapshot.json` for Local Query API multi-backend GET; File POST/DELETE already on API. **Tests:** unit `library-search-and-snapshot.test.js`, `service-worker-open-bookmarks-index.test.js` (`data.q`); composition `library-search-entry.integration.test.js`, `local-query-api-snapshot.integration.test.js`. CITDP: `tied/citdp/CITDP-REQ-BOOKMARK_MANAGER_COMPLETENESS-notes-search-health-local-api.yaml`. **No Playwright E2E**.

- **Local Query API (localhost)** ([REQ-LOCAL_QUERY_API], [ARCH-LOCAL_QUERY_API], [IMPL-LOCAL_QUERY_API]) – Companion Go binary `hoverboard-local-api` binds `127.0.0.1:8787`, bearer token `~/.hoverboard/api-token`, serves File `hoverboard-bookmarks.json` (and optional aggregate snapshot); File POST/DELETE; docs [`docs/LOCAL_API.md`](docs/LOCAL_API.md); Options help; installer copies binary when present. **Tests:** Go `native_host/localapi/api_test.go` (incl. snapshot preference); composition `local-query-api-snapshot.integration.test.js`. CITDP: `tied/citdp/CITDP-REQ-BOOKMARK_MANAGER_COMPLETENESS-notes-search-health-local-api.yaml`.

- **IMPL three-way alignment unit suites** ([PROC-IMPL_CODE_TEST_SYNC]) – New/extended unit coverage for previously empty-traceability Active IMPLs: `logger.test.js`, `debug-logger.test.js`, `dev-command-inspection.test.js`, `popup-message-timeout.test.js`, `popup-move-bookmark.test.js`, `pinboard-service.test.js`, `move-bookmark-ui.test.js`, `ui-action-contract.test.js`, `side-panel-snapshot.test.js`, `theme-and-search-stubs.test.js`; config-manager / helper-path-normalize PROCEDURE gaps; composition `move-bookmark-preferred-backend.integration.test.js` (Save-to→`preferredBackend`). Working: `tied/working/IMPL_THREE_WAY_AUDIT_20260730113900.yaml`. CITDP: `tied/citdp/CITDP-IMPL_THREE_WAY_AUDIT-20260730113900.yaml`.
...
- **`getSelectedStorageBackend` includes Browser** ([IMPL-MOVE_BOOKMARK_UI], [REQ-MOVE_BOOKMARK_STORAGE_UI]) – Preferred-backend allowlist omitted `browser`, so Save-to Browser could not drive `preferredBackend` on save. **Tests:** `move-bookmark-ui.test.js`; composition `move-bookmark-preferred-backend.integration.test.js`.
...
- **Content hover: saveTag / deleteTag payloads vs runtime validation** ([REQ-TAG_INPUT_SANITIZATION], [IMPL-TAG_SYSTEM], [IMPL-RUNTIME_VALIDATION], [IMPL-MESSAGE_HANDLING]) – `hover-system.js` sent `addTag` / `deleteTag` with `{ pin, tag, description }`, which did not match Zod `saveTag` / `deleteTag` schemas (`url`, `value`). **Fix:** send `saveTag` with `{ url: pin.url, value: tag }` and `deleteTag` with `{ url: pin.url, value: tag }`, aligned with `overlay-manager.js` and `src/shared/message-schemas.js`.
...
- **Side panel This Page: three-way tag chip sort** ([REQ-THIS_PAGE_TAG_SORT], [ARCH-THIS_PAGE_TAG_SORT], [IMPL-THIS_PAGE_TAG_SORT], [REQ-SIDE_PANEL_POPUP_EQUIVALENT], [REQ-SUGGESTED_TAGS_FROM_CONTENT]) – **Sort tags** toolbar on the This Page tab: **A–Z** (case-insensitive display order after label casing), **Frequency** (counts from `hoverboard_tag_frequency`; invalid storage coerced to `{}`), **Relevance** (DOM-tier scores via MAIN-world `suggested-tags-main-world-snippet.js` + two-step inject). One shared mode for **Current**, **Recent**, and **Suggested** chips; membership unchanged; default A–Z; not persisted across sessions. **Popup** unchanged when the sort UI is absent. **Tests:** `tag-chip-sort.test.js`, `uIManager-tag-case-folding.test.js`, `popup-suggested-tags.test.js`, `popup-refresh-tag-frequency-map.test.js`, composition `this-page-tag-sort-composition.integration.test.js`, Playwright `extension-this-page-tag-sort.spec.js`. **TIED:** REQ/ARCH/IMPL detail + indexes + `semantic-tokens`; `IMPL-PLAYWRIGHT_E2E_EXTENSION` and REQ-UI_INSPECTION metrics updated. **README:** This Page / Test Coverage sections updated.

### Changed

- **Popup / side panel This Page: suggested tags normalization and load order** ([REQ-SUGGESTED_TAGS_FROM_CONTENT], [REQ-SUGGESTED_TAGS_DEDUPLICATION], [IMPL-SUGGESTED_TAGS], [IMPL-THIS_PAGE_TAG_SORT], [REQ-THIS_PAGE_TAG_SORT]) – MAIN-world extraction results for **Suggested Tags** are normalized in `PopupController`: tag strings are **trimmed**, and empty or whitespace-only labels are **dropped** before filtering out tags already on the current bookmark. **Initial load** awaits **recent tags** before **suggested tags** so Recent chips are populated before suggestions refresh (same flow in popup and side panel). **Tests:** `popup-suggested-tags.test.js`, `this-page-tag-sort-composition.integration.test.js`. **TIED:** IMPL/ARCH/REQ detail, implementation index, and `tied_validate_consistency` aligned ([PROC-AGENT_REQ_CHECKLIST] S12–S14). **README:** This Page notes + test counts.
...
- **Usage tab demo GIF: align with demo_gif_standard** ([IMPL-DEMO_OVERLAY], [PROC-DEMO_RECORDING], [REQ-BOOKMARK_USAGE_TRACKING], [IMPL-BOOKMARK_USAGE_TRACKING_UI], [PROC-TIED_DEV_CYCLE]) – The Usage tab demo ([docs/demo-visit-history.gif](docs/demo-visit-history.gif)) now follows the same standard as Bookmarks and Tabs: **starts** with the standalone Visit History page visible, 1 s no-overlay static frame (frame 0), overlay header rgba(0,0,0,0.78), overlay descriptions 30–50% longer, RATE=1.25, per-step highlights scoped to `#visitHistoryPanel` (toolbar, Most Visited, Recently Visited, Refresh, Navigation Graph), **ends** with a 0.5 s frame showing the Hoverboard icon; GIF build uses 3-part ffmpeg concat (no-overlay 1 s, main 1 fps, end 0.5 s). The workflow seeds usage and navigation-edge records. TIED: IMPL-DEMO_OVERLAY code_locations, essence_pseudocode, traceability.tests, and cross_references updated for Visit History demo.
...
- **Bookmarks demo GIF: start on tab, end card, slower and clearer** ([IMPL-DEMO_OVERLAY], [PROC-DEMO_RECORDING], [REQ-SIDE_PANEL_BROWSER_BOOKMARKS]) – The Browser Bookmarks demo ([docs/demo-browser-bookmarks.gif](docs/demo-browser-bookmarks.gif)) now **starts** with the standalone Browser Bookmarks page visible. The GIF **ends** with a 0.5 s frame showing the Hoverboard icon centered on screen. Overlay descriptions are 30–50% longer; presentation rate is 25% slower (RATE=1.25); overlay header background is slightly more opaque (rgba 0.78). The workflow seeds native Chrome bookmarks. TIED: IMPL-DEMO_OVERLAY essence_pseudocode and code_locations updated.

### Added

- **Demo scripts: By Tag and Tabs highlights; Browser Bookmarks demo GIF** ([IMPL-DEMO_OVERLAY], [PROC-DEMO_RECORDING], [REQ-SIDE_PANEL_TAGS_TREE], [REQ-SIDE_PANEL_BROWSER_BOOKMARKS], [REQ-SIDE_PANEL_BROWSER_TABS]) – **By Tag** ([docs/demo-side-panel-by-tag.gif](docs/demo-side-panel-by-tag.gif)): added highlights for **Bookmarks under selected tags** (#treeContainer) and **Opens in new tab** (first .tree-bookmark-link), plus an extra beat at the end of the animation. **Tabs** ([docs/demo-side-panel-tabs.gif](docs/demo-side-panel-tabs.gif)): every step now has an element highlight (tab bar for steps 1–3, then #browserTabsPanel elements: list, display mode, filter, Copy Records/URLs, etc.). **Browser Bookmarks**: script `scripts/record-demo-side-panel-bookmarks.js` seeds Chrome bookmarks (folder + 5–10 items) and records the standalone page (search, folder, sort, count, click URL); output [docs/demo-browser-bookmarks.gif](docs/demo-browser-bookmarks.gif). TIED: IMPL-DEMO_OVERLAY and PROC-DEMO_RECORDING updated; README documents all five demos.
...
- **Pinboard.in with side panel (Bookmark tab) screenshot** ([IMPL-SCREENSHOT_MODE], [REQ-SIDE_PANEL_POPUP_EQUIVALENT], [IMPL-SIDE_PANEL_BOOKMARK]) – Historical screenshot workflow; superseded by the current side-panel and standalone-tool captures documented in README.
...
- **Local Bookmarks Index – Sticky Table Display and column headers** (`REQ-LOCAL_BOOKMARKS_INDEX`, `ARCH-LOCAL_BOOKMARKS_INDEX`, `IMPL-LOCAL_BOOKMARKS_INDEX`) – Superseded by the fixed head/footer control panels documented in the Unreleased entry above. The former implementation kept the **Table Display** block and **table column headers** at the viewport top after scrolling past Table Display, using `--table-display-sticky-height`, `setTableDisplayStickyHeight`, and `bookmarks-table-sticky.js`.
...
- **Screenshot and placeholder tooling** (`IMPL-SCREENSHOT_MODE`, `IMPL-POPUP_THEME_CSS`) – Tooling to generate reproducible README/marketing screenshots with no live account:
  ...
  - **Playwright script:** `scripts/screenshots-placeholder.js` builds the extension, launches Chrome with the extension loaded, seeds deterministic bookmark, usage, navigation-edge, and native bookmark data, and captures the three side-panel tabs plus the Local Bookmarks Index, standalone Browser Bookmarks, and standalone Visit History pages.
