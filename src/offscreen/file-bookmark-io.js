/**
 * Offscreen document for file bookmark I/O - [IMPL-FILE_BOOKMARK_SERVICE] [ARCH-FILE_BOOKMARK_PROVIDER]
 * Handles READ_FILE_BOOKMARKS and WRITE_FILE_BOOKMARKS messages; reads/writes hoverboard-bookmarks.json
 * using File System Access API with directory handle from IndexedDB.
 * [REQ-FILE_BOOKMARK_STORAGE]
 */

const INDEXEDDB_NAME = 'hoverboard_file_storage'
const STORE_NAME = 'handles'
const HANDLE_KEY = 'directory_handle'
const FILENAME = 'hoverboard-bookmarks.json'
const ARCHIVE_FILENAME = 'hoverboard-page-archives.json'
const FILE_FORMAT_VERSION = 1

function getDb () {
  return new Promise((resolve, reject) => {
    const req = indexedDB.open(INDEXEDDB_NAME, 1)
    req.onerror = () => reject(req.error)
    req.onsuccess = () => resolve(req.result)
    req.onupgradeneeded = (e) => {
      e.target.result.createObjectStore(STORE_NAME)
    }
  })
}

async function getDirectoryHandle () {
  const db = await getDb()
  return new Promise((resolve, reject) => {
    const tx = db.transaction(STORE_NAME, 'readonly')
    const req = tx.objectStore(STORE_NAME).get(HANDLE_KEY)
    req.onsuccess = () => resolve(req.result || null)
    req.onerror = () => reject(req.error)
  })
}

async function readFile (dirHandle, filename = FILENAME) {
  try {
    const fileHandle = await dirHandle.getFileHandle(filename, { create: false })
    const file = await fileHandle.getFile()
    const text = await file.text()
    const data = JSON.parse(text || '{}')
    if (filename === FILENAME && (!data.bookmarks || typeof data.bookmarks !== 'object')) {
      return { version: FILE_FORMAT_VERSION, bookmarks: {} }
    }
    if (filename === ARCHIVE_FILENAME && (!data.archives || typeof data.archives !== 'object')) {
      return { version: FILE_FORMAT_VERSION, archives: {}, screenshots: {} }
    }
    return filename === ARCHIVE_FILENAME
      ? { version: data.version || FILE_FORMAT_VERSION, archives: data.archives, screenshots: data.screenshots || {} }
      : { version: data.version || FILE_FORMAT_VERSION, bookmarks: data.bookmarks }
  } catch (e) {
    if (e.name === 'NotFoundError') {
      return filename === ARCHIVE_FILENAME
        ? { version: FILE_FORMAT_VERSION, archives: {}, screenshots: {} }
        : { version: FILE_FORMAT_VERSION, bookmarks: {} }
    }
    throw e
  }
}

async function writeFile (dirHandle, data, filename = FILENAME) {
  const fileHandle = await dirHandle.getFileHandle(filename, { create: true })
  const writable = await fileHandle.createWritable()
  const json = filename === ARCHIVE_FILENAME
    ? JSON.stringify({ version: data.version || FILE_FORMAT_VERSION, archives: data.archives || {}, screenshots: data.screenshots || {} }, null, 2)
    : JSON.stringify({ version: data.version || FILE_FORMAT_VERSION, bookmarks: data.bookmarks || {} }, null, 2)
  await writable.write(json)
  await writable.close()
}

/**
 * [IMPL-FILE_BOOKMARK_SERVICE] [REQ-FILE_BOOKMARK_STORAGE]
 * Handles READ_FILE_BOOKMARKS and WRITE_FILE_BOOKMARKS. Exported for unit tests.
 * @param {{ type: string, data?: object }} message
 * @param {(r: object) => void} sendResponse
 * @param {{ getDirectoryHandle: () => Promise<unknown>, readFile: (h: unknown) => Promise<{ version: number, bookmarks: object }>, writeFile: (h: unknown, data: object) => Promise<void> }} io - Optional; defaults to real getDirectoryHandle/readFile/writeFile
 * @returns {boolean} true if async response expected
 */
export function handleOffscreenMessage (message, sendResponse, io = { getDirectoryHandle, readFile, writeFile }) {
  const type = message?.type
  const { getDirectoryHandle: getHandle, readFile: read, writeFile: write } = io
  if (type === 'READ_FILE_BOOKMARKS') {
    getHandle()
      .then((handle) => {
        if (!handle) {
          sendResponse({ error: 'NO_HANDLE', data: null })
          return
        }
        return read(handle, FILENAME).then((data) => sendResponse({ error: null, data }))
      })
      .catch((e) => sendResponse({ error: e.message || 'READ_FAILED', data: null }))
    return true
  }
  if (type === 'WRITE_FILE_BOOKMARKS') {
    getHandle()
      .then((handle) => {
        if (!handle) {
          sendResponse({ error: 'NO_HANDLE', success: false })
          return
        }
        return write(handle, message.data || {}, FILENAME).then(() => sendResponse({ error: null, success: true }))
      })
      .catch((e) => sendResponse({ error: e.message || 'WRITE_FAILED', success: false }))
    return true
  }
  if (type === 'READ_FILE_ARCHIVES') {
    getHandle()
      .then((handle) => {
        if (!handle) {
          sendResponse({ error: 'NO_HANDLE', data: null })
          return
        }
        return read(handle, ARCHIVE_FILENAME).then((data) => sendResponse({ error: null, data }))
      })
      .catch((e) => sendResponse({ error: e.message || 'READ_FAILED', data: null }))
    return true
  }
  if (type === 'WRITE_FILE_ARCHIVES') {
    getHandle()
      .then((handle) => {
        if (!handle) {
          sendResponse({ error: 'NO_HANDLE', success: false })
          return
        }
        return write(handle, message.data || {}, ARCHIVE_FILENAME).then(() => sendResponse({ error: null, success: true }))
      })
      .catch((e) => sendResponse({ error: e.message || 'WRITE_FAILED', success: false }))
    return true
  }
  return false
}

chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  return handleOffscreenMessage(message, sendResponse)
})
