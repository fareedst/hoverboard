 ...
 */
import { hashArchiveContent, normalizeArchiveUrl } from './page-capture.js'

export const DEFAULT_SCREENSHOT_LIMITS = Object.freeze({
...
export function validateScreenshotRequest ({ url, storage, options = {}, limits = DEFAULT_SCREENSHOT_LIMITS } = {}) {
  if (!['local', 'file'].includes(String(storage || '').toLowerCase())) return { valid: false, code: 'UnsupportedBackend' }
  if (!/^https?:/i.test(normalizeArchiveUrl(url))) return { valid: false, code: 'RestrictedUrl' }
  const width = Number(options.width || 0)
  const height = Number(options.height || 0)
  if (width > limits.maxWidth || height > limits.maxHeight) return { valid: false, code: 'TooLarge' }
  return { valid: true }
}

/**
 * [IMPL-PAGE_SCREENSHOT_ARCHIVE] [ARCH-PAGE_SCREENSHOT_ARCHIVE] [REQ-PAGE_SCREENSHOT_ARCHIVE]
 * Convert a browser capture data URL into a bounded, separately addressable screenshot artifact.
 */
export function normalizeScreenshotArtifact ({ url, dataUrl, storage, options = {}, capturedAt, limits = DEFAULT_SCREENSHOT_LIMITS } = {}) {
  const validation = validateScreenshotRequest({ url, storage, options, limits })
  ...
  if (bytes > limits.maxBytes) return { success: false, code: 'TooLarge' }
  const normalizedUrl = normalizeArchiveUrl(url)
  return {
    success: true,
    artifact: {
      artifactId: hashArchiveContent(`${normalizedUrl}:${dataUrl}`),
      url: normalizedUrl,
      version: 1,
      capturedAt: capturedAt || new Date().toISOString(),
      format: dataUrl.slice(5, dataUrl.indexOf(';')).toLowerCase(),
      bytes,
      width: Number(options.width || 0),
      height: Number(options.height || 0),
      fullPage: options.fullPage === true,
      contentHash: hashArchiveContent(dataUrl),
      dataUrl,
      status: 'available',
      storage
    }
