 ...
 */
const UNSAFE_ELEMENTS = 'script,style,iframe,object,embed,form,base,link,meta,template'
const UNSAFE_PROTOCOL = /^(?:javascript|vbscript|file|chrome|moz-extension):/i

function getDocumentForHtml (html, documentLike) {
  if (documentLike?.implementation?.createHTMLDocument) {
    const doc = documentLike.implementation.createHTMLDocument('Hoverboard archive')
...
function isSafeResourceUrl (value) {
  ...
  return /^https?:/i.test(trimmed) && !UNSAFE_PROTOCOL.test(trimmed)
}

/**
 * [IMPL-PAGE_ARCHIVE_STORAGE] [ARCH-PAGE_ARCHIVE_STORAGE] [REQ-PAGE_ARCHIVE_STORAGE]
 * Strip executable elements, event handlers, unsafe URLs, and inline CSS before Reader rendering.
 */
export function sanitizeArchiveHtml (html, documentLike = globalThis.document) {
  const doc = getDocumentForHtml(html, documentLike)
  if (!doc?.body) {
    // [IMPL-PAGE_ARCHIVE_STORAGE] [ARCH-PAGE_ARCHIVE_STORAGE] [REQ-PAGE_ARCHIVE_STORAGE]
    // Service workers do not expose DOMParser; retain a conservative fallback sanitizer for that boundary.
    return String(html || '')
      .replace(/<\s*(script|style|iframe|object|embed|form|base|link|meta|template)\b[^>]*>[\s\S]*?<\s*\/\s*\1\s*>/gi, '')
      .replace(/\s+on[a-z-]+\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+)/gi, '')
      .replace(/\s+style\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+)/gi, '')
      .replace(/\s+(href|src|action|formaction)\s*=\s*(['"])\s*(?:javascript|vbscript|file|chrome|moz-extension):[\s\S]*?\2/gi, '')
  }

  doc.body.querySelectorAll(UNSAFE_ELEMENTS).forEach((node) => node.remove())
  doc.body.querySelectorAll('*').forEach((element) => {
    for (const attribute of Array.from(element.attributes)) {
      ...
      } else if (['href', 'src', 'action', 'formaction'].includes(name) && !isSafeResourceUrl(value)) {
        element.removeAttribute(attribute.name)
      }
    }
