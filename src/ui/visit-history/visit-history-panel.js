 ...
 * Surface 3 – Visit History standalone page. Most Visited, Recently Visited, Navigation Graph.
 *
 * ## INIT_VISIT_HISTORY_PAGE
 *
 * - [IMPL-BOOKMARK_USAGE_TRACKING_UI] [ARCH-BOOKMARK_USAGE_TRACKING_UI] [REQ-BOOKMARK_USAGE_TRACKING] How: fetch stats and graph; render three sections; bind Refresh. Not a side-panel tab.
 * - Contract:
 *   - INPUT: { panelEl?, sendMessage? }
 *   - PRE: #visitHistoryPanel (or panelEl) in DOM
 *   - OUTPUT: sections rendered; Refresh bound
 *   - POST: success => lists/graph reflect message replies
 *   - EFFECTS: IO (runtime.sendMessage)
 *   - TERMINATION: total
 * - PROCEDURE: INIT_VISIT_HISTORY_PAGE
 *   - send getBookmarkUsageStats({ n: 10 }), getBookmarkNavigationGraph()
 *   - render Most Visited (mostFrequent), Recently Visited (mostRecent), Navigation Graph (edges grouped by sourceUrl)
 *
...
/**
 * [IMPL-BOOKMARK_USAGE_TRACKING_UI] [ARCH-BOOKMARK_USAGE_TRACKING_UI] [REQ-BOOKMARK_USAGE_TRACKING]
 * Initialize the Visit History page: fetch stats and graph, render three sections, bind Refresh.
 * @param {{ panelEl?: HTMLElement, sendMessage?: (msg: object) => Promise<object> }} options
 */
export function initVisitHistoryPage (options = {}) {
  const panelEl = options.panelEl || document.getElementById('visitHistoryPanel')
...
/** @deprecated Use initVisitHistoryPage */
export function initUsageTab (options = {}) {
  const panelEl = options.panelEl || document.getElementById('visitHistoryPanel') || document.getElementById('usagePanel')
  return initVisitHistoryPage({ ...options, panelEl })
}
