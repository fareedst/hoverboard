 ...
 */
import { normalizeScreenshotArtifact, validateScreenshotRequest } from '../../src/features/archive/page-screenshot-capture.js'
...
import { captureProductScreenshot } from '../../src/features/archive/browser-screenshot-capture.js'

const image = 'data:image/png;base64,' + 'A'.repeat(128)

describe('Page screenshot archive [REQ-PAGE_SCREENSHOT_ARCHIVE]', () => {
  test('uses the dedicated browser boundary for viewport capture', async () => {
    const tabs = { captureVisibleTab: jest.fn().mockResolvedValue(image) }
    await expect(captureProductScreenshot({
      tabs,
      windowId: 7,
      options: { format: 'png' }
    ...
    expect(tabs.captureVisibleTab).toHaveBeenCalledWith(7, {
      format: 'png',
      quality: undefined
    })
  })

  test('does not mislabel viewport capture as full-page', async () => {
    await expect(captureProductScreenshot({
      tabs: { captureVisibleTab: jest.fn() },
      options: { fullPage: true }
    })).resolves.toEqual({
      success: false,
      code: 'FullPageCaptureUnavailable'
    })
  })

  test('accepts an explicitly supplied full-page capture capability', async () => {
    const captureFullPage = jest.fn().mockResolvedValue(image)
    await expect(captureProductScreenshot({
      options: { fullPage: true },
      captureFullPage
    })).resolves.toMatchObject({
    ...
    expect(captureFullPage).toHaveBeenCalledWith({
      format: 'png',
      quality: undefined,
      fullPage: true
    })
  ...
  test('accepts Local/File HTTP(S) capture and keeps demo mode independent', () => {
    expect(validateScreenshotRequest({ url: 'https://example.com', storage: 'local' }).valid).toBe(true)
    expect(normalizeScreenshotArtifact({
      url: 'https://example.com',
      storage: 'file',
      dataUrl: image,
      options: { fullPage: true, width: 1200, height: 800 },
  ...
  test('rejects restricted URLs, unsupported backends, and invalid image output', () => {
    expect(validateScreenshotRequest({ url: 'chrome://settings', storage: 'local' })).toEqual({ valid: false, code: 'RestrictedUrl' })
    expect(validateScreenshotRequest({ url: 'https://example.com', storage: 'sync' })).toEqual({ valid: false, code: 'UnsupportedBackend' })
    expect(normalizeScreenshotArtifact({ url: 'https://example.com', storage: 'local', dataUrl: 'not-an-image' })).toEqual({
      success: false,
      code: 'CaptureFailed'
    })
  })

  test('persists screenshot separately and deletes it with the page archive', async () => {
    const adapter = new InMemoryPageArchiveStorageAdapter()
    const store = new PageScreenshotStore({ localAdapter: adapter, fileAdapter: new InMemoryPageArchiveStorageAdapter() })
    const result = await store.saveScreenshot('https://example.com', 'local', {
      artifactId: 'shot-1',
      dataUrl: image,
      capturedAt: '2026-07-31T12:00:00.000Z'
    })
    expect(result.success).toBe(true)
    expect((await store.listScreenshots('local'))[0].artifactId).toBe('shot-1')
    await expect(store.deleteScreenshots('https://example.com', 'local')).resolves.toEqual({ success: true })
    expect(await store.listScreenshots('local')).toEqual([])
  })

  test('replaces the current URL artifact instead of accumulating stale captures', async () => {
    const adapter = new InMemoryPageArchiveStorageAdapter()
    const store = new PageScreenshotStore({ localAdapter: adapter, fileAdapter: new InMemoryPageArchiveStorageAdapter() })

    await store.saveScreenshot('https://example.com/replace-shot', 'local', {
      artifactId: 'shot-old',
      dataUrl: image,
      capturedAt: '2026-07-31T12:00:00.000Z'
    })
    const replacement = await store.saveScreenshot('https://example.com/replace-shot', 'local', {
      artifactId: 'shot-new',
      dataUrl: image + 'B',
      capturedAt: '2026-07-31T12:01:00.000Z'
    })

    expect(replacement).toMatchObject({ success: true, artifact: { artifactId: 'shot-new', version: 2 } })
    expect(await store.listScreenshots('local')).toEqual([
      expect.objectContaining({ artifactId: 'shot-new', version: 2 })
    ])
  })

  test('keeps the prior screenshot when replacement storage fails [REQ-PAGE_SCREENSHOT_ARCHIVE]', async () => {
    const adapter = new InMemoryPageArchiveStorageAdapter()
    const store = new PageScreenshotStore({ localAdapter: adapter, fileAdapter: new InMemoryPageArchiveStorageAdapter() })
    await store.saveScreenshot('https://example.com/write-failure', 'local', {
      artifactId: 'shot-prior',
      dataUrl: image
    })
    adapter.writeArchiveFile = jest.fn().mockRejectedValue(new Error('disk full'))

    await expect(store.saveScreenshot('https://example.com/write-failure', 'local', {
      artifactId: 'shot-next',
      dataUrl: image + 'B'
    })).resolves.toMatchObject({ success: false, code: 'StorageFailed' })
    expect((await adapter.readArchiveFile()).screenshots).toEqual({
      'shot-prior': expect.objectContaining({ artifactId: 'shot-prior' })
    })
  })
})
