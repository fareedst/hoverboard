...
} from '../../src/shared/suggested-tag-state.js'

describe('CLASSIFY_SIDE_PANEL_SUGGESTED_TAG_STATE [REQ-SUGGESTED_TAGS_DEDUPLICATION]', () => {
  it('identifies exact-case matches and case mismatches for three-state rendering', () => {
    expect(classifySuggestedTagState('React', ['React', 'browser'])).toEqual({
      state: 'case-match',
      suggestedTag: 'React',
      matchedTag: 'React'
    })
  ...
  it('preserves source casing for absent suggestions', () => {
    expect(classifySuggestedTagState('JavaScript', ['browser'])).toEqual({
      state: 'absent',
      suggestedTag: 'JavaScript',
      matchedTag: null
    })
  })

  it('classifies the adjusted value while preserving the raw Suggested Tag identity', () => {
    expect(classifySuggestedTagState('React', ['REACT'], 'REACT')).toEqual({
      state: 'case-match',
      suggestedTag: 'React',
      matchedTag: 'REACT'
    })
  ...
  it('rejects unsafe page-derived values', () => {
    expect(sanitizeSuggestedTag('<script>')).toBeNull()
    expect(classifySuggestedTagState('<script>', [])).toBeNull()
  })
})

describe('REPLACE_SUGGESTED_TAG_IN_PLACE [REQ-SUGGESTED_TAGS_CASE_PRESERVATION]', () => {
  it('replaces the first case-insensitive match and collapses duplicate variants', () => {
    const currentTags = ['unrelated', 'React', 'react', 'later']

    expect(replaceTagInPlace(currentTags, 'React', 'REACT')).toEqual({
      ok: true,
      tags: ['unrelated', 'REACT', 'later'],
      replacedTag: 'React'
    })
    expect(currentTags).toEqual(['unrelated', 'React', 'react', 'later'])
  })

  it('preserves unrelated tags and returns no-mutating failures', () => {
    const currentTags = ['React', 'unrelated']

    expect(replaceTagInPlace(currentTags, 'Vue', 'REACT')).toEqual({
      ok: false,
      tags: ['React', 'unrelated'],
      reason: 'match_not_found'
    })
    expect(replaceTagInPlace(currentTags, 'React', '<script>')).toEqual({
      ok: false,
      tags: ['React', 'unrelated'],
      reason: 'invalid_replacement'
    })
    expect(currentTags).toEqual(['React', 'unrelated'])
  })
})
