 ...
 */
import {
  createControlTabState,
  isValidControlGroup,
  selectControlGroup
} from '../../src/ui/bookmarks-table/bookmarks-table-controls.js'

describe('createControlTabState [REQ-LOCAL_BOOKMARKS_INDEX]', () => {
  test('defaults head to Stores and footer collapsed', () => {
    expect(createControlTabState()).toEqual({
      head: 'stores',
      footer: null
    })
  })
...
describe('selectControlGroup [IMPL-LOCAL_BOOKMARKS_INDEX] [REQ-LOCAL_BOOKMARKS_INDEX]', () => {
  test('changes head and footer independently', () => {
    const state = createControlTabState()

    const withFilters = selectControlGroup(state, 'head', 'show-only')
    const withExport = selectControlGroup(withFilters, 'footer', 'export')
  ...
  test('opens a footer group from the collapsed state', () => {
    const state = createControlTabState()

    expect(selectControlGroup(state, 'footer', 'import')).toEqual({
      head: 'stores',
      footer: 'import'
    })
  })

  test('collapses the active footer group on repeat activation', () => {
    const state = selectControlGroup(createControlTabState(), 'footer', 'import')
  ...
  test('ignores an invalid group without changing state', () => {
    const state = createControlTabState()

    expect(selectControlGroup(state, 'head', 'actions')).toBe(state)
    expect(selectControlGroup(state, 'footer', 'stores')).toBe(state)
    expect(selectControlGroup(state, 'unknown', 'stores')).toBe(state)
  })
})

describe('isValidControlGroup [REQ-LOCAL_BOOKMARKS_INDEX]', () => {
  test('recognizes head groups', () => {
    expect(isValidControlGroup('head', 'stores')).toBe(true)
    expect(isValidControlGroup('head', 'show-only')).toBe(true)
    expect(isValidControlGroup('head', 'hide')).toBe(true)
    expect(isValidControlGroup('head', 'table-display')).toBe(true)
  })

  test('recognizes footer groups', () => {
    expect(isValidControlGroup('footer', 'actions')).toBe(true)
    expect(isValidControlGroup('footer', 'import')).toBe(true)
    expect(isValidControlGroup('footer', 'export')).toBe(true)
  })

  test('rejects groups from the wrong region', () => {
    expect(isValidControlGroup('head', 'export')).toBe(false)
    expect(isValidControlGroup('footer', 'stores')).toBe(false)
    expect(isValidControlGroup('head', '')).toBe(false)
  })
})
