 ...
 * Shared Save to controls preserve ordinary preferredBackend routing; new Read Later saves resolve Browser to a metadata-capable backend.
 *
 * ## IS_PERSISTED_BOOKMARK
 * - [IMPL-MOVE_BOOKMARK_UI] [ARCH-MOVE_BOOKMARK_UI] [REQ-READ_LATER_BROWSER_FALLBACK] URL-only currentPin stubs are new bookmarks; persisted identity requires time.
 *
 * ## RESOLVE_READ_LATER_BACKEND
 * - [IMPL-MOVE_BOOKMARK_UI] [ARCH-MOVE_BOOKMARK_UI] [REQ-STORAGE_MODE_DEFAULT] [REQ-READ_LATER_BROWSER_FALLBACK] Preserve pinboard/local/file/sync; resolve Browser through ConfigManager and fall back to local.
 *
 * ## CREATE_READ_LATER_BOOKMARK
 * - [IMPL-MOVE_BOOKMARK_UI] [ARCH-MOVE_BOOKMARK_UI] [REQ-MOVE_BOOKMARK_STORAGE_UI] [REQ-READ_LATER_BROWSER_FALLBACK] Reuse createBookmark with toread=yes and the resolved preferredBackend.
 *
 * ## APPLY_READ_LATER_RESULT
 * - [IMPL-MOVE_BOOKMARK_UI] [ARCH-MOVE_BOOKMARK_UI] [REQ-READ_LATER_BROWSER_FALLBACK] Update Save to and show fallback information only after persistence; archive status is independent.
 *
 * ## READ_LATER_EVENT_BINDING
 * - [IMPL-MOVE_BOOKMARK_UI] [ARCH-MOVE_BOOKMARK_UI] [REQ-READ_LATER_BROWSER_FALLBACK] Shared UIManager readLater event invokes PopupController.handleReadLater.
 *
 * ## READ_LATER_ACTION
 * - [IMPL-MOVE_BOOKMARK_UI] [ARCH-MOVE_BOOKMARK_UI] [REQ-READ_LATER_BROWSER_FALLBACK] Existing bookmarks use the current toggle path; fallback applies only to new saves.
 *
...
describe('BookmarkRouter [IMPL-BOOKMARK_ROUTER] [ARCH-STORAGE_INDEX_AND_ROUTER] [REQ-PER_BOOKMARK_STORAGE_BACKEND]', () => {
  ...
  test('selected-backend lookup bypasses aggregate 2C lookup [IMPL-PAGE_ARCHIVE_BOOKMARK_ASSOCIATION]', async () => {
    // [IMPL-PAGE_ARCHIVE_BOOKMARK_ASSOCIATION] [ARCH-PAGE_ARCHIVE_BOOKMARK_ASSOCIATION] [REQ-PAGE_ARCHIVE_STORAGE] Resolve explicit backend ownership without aggregate URL lookup.
    const url = 'https://example.com/selected'
    await file.saveBookmark({ url, description: 'File record', time: '2026-08-01T10:00:00.000Z' })

    const result = await router.getBookmarkForBackend(url, 'file')

    expect(result.description).toBe('File record')
    expect(file.getAllBookmarks).toHaveBeenCalled()
    expect(pinboard.getBookmarkForUrl).not.toHaveBeenCalledWith(url, '')
    expect(local.getBookmarkForUrl).not.toHaveBeenCalledWith(url, '')
    expect(sync.getBookmarkForUrl).not.toHaveBeenCalledWith(url, '')
  })
