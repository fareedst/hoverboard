 ...
 * Unit: UNWRAP_MESSAGE_RESPONSE — a missing (null) runtime reply must be observable, never a thrown TypeError.
 */
...
import {
  ...
  readMessageResponse
} from '../../src/shared/message-response.js'
import { setEnabled, clear, getLastActions } from '../../src/shared/ui-inspector.js'

describe('[IMPL-MESSAGE_HANDLING] unwrapMessageResponse', () => {
  test('missing responses are detected', () => {
    expect(isMissingMessageResponse(null)).toBe(true)
    expect(isMissingMessageResponse(undefined)).toBe(true)
    expect(isMissingMessageResponse({ success: true, data: {} })).toBe(false)
  })

  test('null and undefined unwrap to null instead of throwing', () => {
    expect(unwrapMessageResponse(null)).toBeNull()
    expect(unwrapMessageResponse(undefined)).toBeNull()
  })

  test('success wrapper unwraps to data', () => {
  ...
  test('plain payload without success flag is returned as-is', () => {
    const payload = { tabId: 3 }
    expect(unwrapMessageResponse(payload)).toBe(payload)
  })
})

describe('[IMPL-UI_INSPECTOR] readMessageResponse instrumentation', () => {
  beforeEach(() => {
    setEnabled(true)
    clear()
  })
  ...
  test('missing reply returns null and records messageResponseMissing', () => {
    expect(readMessageResponse(null, 'getTabId')).toBeNull()

    const missing = getLastActions().filter((a) => a.actionId === 'messageResponseMissing')
    expect(missing).toHaveLength(1)
    expect(missing[0].payload).toMatchObject({ type: 'getTabId' })
    expect(missing[0].surface).toBe('content')
  })

  test('successful reply returns payload and records nothing', () => {
    expect(readMessageResponse({ success: true, data: { tabId: 7 } }, 'getTabId')).toEqual({ tabId: 7 })
    expect(getLastActions().filter((a) => a.actionId === 'messageResponseMissing')).toHaveLength(0)
  })
})
