...
describe('[IMPL-SUGGESTED_TAGS] [IMPL-THIS_PAGE_TAG_SORT] [REQ-SUGGESTED_TAGS_FROM_CONTENT] Popup suggested tags', () => {
  ...
  describe('loadSuggestedTags injectable URL [REQ-SUGGESTED_TAGS_FROM_CONTENT]', () => {
    ...
    test('[REQ-SIDE_PANEL_POPUP_EQUIVALENT] side-panel classification renders all states and preserves matched metadata', async () => {
      uiManager.container = {}
      popupController.currentTab = { id: 13, url: 'https://example.com' }
      popupController.currentPin = { tags: ['React', 'unrelated'] }
      popupController.normalizeTags = jest.fn((tags) => tags || [])
      mockMainWorldExtract(['React', 'react', 'JavaScript'])
    ...
    test('[REQ-SIDE_PANEL_POPUP_EQUIVALENT] side-panel classification compares the adjusted Tag labels value', async () => {
      uiManager.container = {}
      uiManager.tagCaseFoldingMode = 'upper'
      popupController.currentTab = { id: 14, url: 'https://example.com' }
      popupController.currentPin = { tags: ['REACT'] }
      popupController.normalizeTags = jest.fn((tags) => tags || [])
      mockMainWorldExtract(['React'])
