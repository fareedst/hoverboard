 ...
 */
jest.mock('../../src/features/content/overlay-manager.js', () => ({
  OverlayManager: jest.fn().mockImplementation(() => ({
    config: {},
    isVisible: false,
    applyTransparencyMode: jest.fn()
  }))
}))
...
jest.mock('../../src/features/content/dom-utils.js', () => ({
  DOMUtils: jest.fn().mockImplementation(() => ({
    waitForDOM: jest.fn().mockResolvedValue(undefined)
  }))
}))

jest.mock('../../src/features/ai/readability-extract.js', () => ({
  extractPageContent: jest.fn(() => ({ title: 't', text: '' }))
}))

import { HoverboardContentScript } from '../../src/features/content/content-main.js'
...
describe('[IMPL-MESSAGE_HANDLING] content-main null reply fallback', () => {
  /** @type {HoverboardContentScript} */
  let script

  beforeEach(() => {
    setEnabled(true)
    clear()
    jest.spyOn(HoverboardContentScript.prototype, 'init').mockResolvedValue(undefined)
    script = new HoverboardContentScript()
    script.messageClient = { sendMessage: jest.fn().mockResolvedValue(null) }
    script.tabId = null
    script.config = null
    script.currentBookmark = { keep: true }
    script.pageUrl = 'https://example.com/page'
    script.pageTitle = 'Example'
  })
  ...
  test('initializeTabId keeps tabId null and records messageResponseMissing', async () => {
    await expect(script.initializeTabId()).resolves.toBeUndefined()
    expect(script.tabId).toBeNull()
    const missing = getLastActions().filter((a) => a.actionId === 'messageResponseMissing')
    expect(missing.some((a) => a.payload?.type === MESSAGE_TYPES.GET_TAB_ID)).toBe(true)
  })

  test('loadConfiguration falls back to getDefaultConfig on null reply', async () => {
    await script.loadConfiguration()
    expect(script.config).toEqual(expect.objectContaining({
      showHoverOnPageLoad: false,
      overlayTransparencyMode: 'nearly-transparent'
    }))
    const missing = getLastActions().filter((a) => a.actionId === 'messageResponseMissing')
    expect(missing.some((a) => a.payload?.type === MESSAGE_TYPES.GET_OPTIONS)).toBe(true)
  })

  test('loadCurrentPageData keeps prior bookmark and does not throw on null reply', async () => {
    const prior = script.currentBookmark
    await expect(script.loadCurrentPageData()).resolves.toBeUndefined()
    expect(script.currentBookmark).toBe(prior)
    const missing = getLastActions().filter((a) => a.actionId === 'messageResponseMissing')
    expect(missing.some((a) => a.payload?.type === MESSAGE_TYPES.GET_CURRENT_BOOKMARK)).toBe(true)
  })
})
