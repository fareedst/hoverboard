...
A short demo animation walks through the **This Page** tab with highlighted sections and overlay descriptions: **Quick Actions** (Show Hover, Toggle Privacy, Read Later, Delete), **Save to** (Pinboard, File, Local, Sync), **Tag with AI** (AI-suggested tags; set API key in Options), **Recent Tags** and **Suggested Tags** (seeded in demo mode so both sections appear), **Search Tabs**, and the **footer** (Reload, Options, Bookmarks index, By Tag, Browser bookmark import). The GIF **starts** with a 1 s static frame (no overlay), uses presentation rate 25% slower (RATE=1.25), overlay header rgba(0,0,0,0.78), and descriptions 30–50% longer; it **ends** with a 0.5 s frame showing the Hoverboard icon. Produce the GIF with `node scripts/record-demo-side-panel-this-page.js`. **Standard for all demo GIFs** (timing, interstitial, opacity, build): see [Demo GIF standard](docs/development/demo-gif-standard.md) and [IMPL-DEMO_OVERLAY](tied/implementation-decisions/IMPL-DEMO_OVERLAY.yaml) `demo_gif_standard`.
...
A short demo animation shows using the Tabs tab to find and export a set of pages (filter then **Copy Records** or **Copy URLs**). The GIF **starts** with a 1 s static frame (optionally with the Tabs tab already visible; the script persists the tab choice before opening the panel), then overlay steps with per-step highlights (tab bar, list, filter, display mode, Copy buttons, etc.). Presentation rate is 25% slower (RATE=1.25); overlay descriptions are 30–50% longer; the GIF **ends** with a half-second frame of the Hoverboard icon. Produce the GIF with `node scripts/record-demo-side-panel-tabs.js` (or `npm run record:demo:tabs`). **Standard for all demo GIFs** (timing, interstitial, opacity, build): see [Demo GIF standard](docs/development/demo-gif-standard.md) and [IMPL-DEMO_OVERLAY](tied/implementation-decisions/IMPL-DEMO_OVERLAY.yaml) `demo_gif_standard`.
...
A short demo shows the **Bookmarks** tab: the animation **starts** with the Bookmarks tab already visible (the script persists the tab choice before opening the panel), then walks through the list, search, folder filter, sort, match count, and clicking a URL (opens in new tab). Overlay descriptions are slightly longer for context, and the presentation rate is 25% slower. The GIF **ends** with a half-second frame of the Hoverboard icon centered on screen. The script seeds a "Hoverboard Demo" folder with several bookmarks so the GIF uses medium-complexity data. Produce the GIF with `node scripts/record-demo-side-panel-bookmarks.js`. **Standard for all demo GIFs** (timing, interstitial, opacity, build): see [Demo GIF standard](docs/development/demo-gif-standard.md) and [IMPL-DEMO_OVERLAY](tied/implementation-decisions/IMPL-DEMO_OVERLAY.yaml) `demo_gif_standard`.
...
- **Extension commands (global shortcuts):** Assign keyboard shortcuts in Chrome at `chrome://extensions/shortcuts`. Up to four commands can have suggested shortcuts; by default: **Ctrl+Shift+1** (side panel – This Page tab), **Ctrl+Shift+2** (side panel – By Tag tab), **Ctrl+Shift+3** (side panel – Tabs tab), **Ctrl+Shift+B** (open bookmarks index). Other commands (open side panel, options, import, open side panel – Bookmarks tab) have no default shortcut; you can assign any.
