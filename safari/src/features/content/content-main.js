...
class HoverboardContentScript {
  ...
  async handleMessage (message, sender, sendResponse) {
    try {
      ...
      // [SAFARI-EXT-COMM-001] Enhanced Safari-specific message processing
      if (this.isSafari) {
        message = this.processSafariMessage(message, sender)
        console.log('[SAFARI-EXT-COMM-001] Processed Safari message:', message)
      }
  ...
  // [SAFARI-EXT-COMM-001] Safari-specific message processing
  processSafariMessage(message, sender) {
    try {
      console.log('[SAFARI-EXT-COMM-001] Processing Safari message:', message)

      // [SAFARI-EXT-COMM-001] Validate incoming message
      if (!message || typeof message !== 'object') {
        throw new Error('[SAFARI-EXT-COMM-001] Invalid message: must be an object')
      }

      if (!message.type || typeof message.type !== 'string') {
        throw new Error('[SAFARI-EXT-COMM-001] Invalid message: type is required and must be a string')
      }

      // [SAFARI-EXT-COMM-001] Add Safari-specific sender information
      const processedMessage = {
        ...message,
        safariInfo: {
          extensionId: safari?.extension?.baseURI || 'unknown',
          timestamp: Date.now(),
      ...
      // [SAFARI-EXT-COMM-001] Add Safari-specific performance monitoring
      if (this.performanceMonitor) {
        this.performanceMonitor.recordMessageProcessing(message.type)
      }

      console.log('[SAFARI-EXT-COMM-001] Safari message processed successfully:', processedMessage)
      return processedMessage
    } catch (error) {
      console.error('[SAFARI-EXT-COMM-001] Safari message processing error:', error)
      throw error
    }
  }

  // [SAFARI-EXT-COMM-001] Safari-specific error handling
  async handleSafariError(error) {
    try {
      console.error('[SAFARI-EXT-COMM-001] Safari error occurred:', error)

      // [SAFARI-EXT-COMM-001] Increment error recovery attempts
      this.errorRecoveryAttempts++

      if (this.errorRecoveryAttempts <= this.maxErrorRecoveryAttempts) {
        console.log(`[SAFARI-EXT-COMM-001] Attempting error recovery (${this.errorRecoveryAttempts}/${this.maxErrorRecoveryAttempts})`)

        // [SAFARI-EXT-COMM-001] Reinitialize components on error
        if (this.messageClient) {
          this.messageClient = new MessageClient()
        }

        if (this.overlayManager) {
          this.overlayManager = new OverlayManager(document, this.safariConfig)
        }

        // [SAFARI-EXT-COMM-001] Wait before retry
        await new Promise(resolve => setTimeout(resolve, 1000))

        console.log('[SAFARI-EXT-COMM-001] Error recovery completed')
      } else {
        console.error('[SAFARI-EXT-COMM-001] Max error recovery attempts reached')
        this.errorRecoveryAttempts = 0
      }
    } catch (recoveryError) {
      console.error('[SAFARI-EXT-COMM-001] Error recovery failed:', recoveryError)
    }
  }
}
