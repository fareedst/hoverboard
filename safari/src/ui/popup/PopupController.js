...
export class PopupController {
  ...
  async sendToTab (message) {
    ...
    // [SAFARI-EXT-WINDOW-001] Safari-specific message enhancement
    const enhancedMessage = {
      ...message,
      timestamp: Date.now(),
      platform: 'safari',
      popupId: this.popupId || 'unknown'
    }

    // [SAFARI-EXT-WINDOW-001] Add Safari-specific sender information
    if (typeof safari !== 'undefined') {
      enhancedMessage.safariInfo = {
    ...
    console.log('[SAFARI-EXT-WINDOW-001] Sending enhanced message to tab:', enhancedMessage)

    return new Promise((resolve, reject) => {
      chrome.tabs.sendMessage(this.currentTab.id, enhancedMessage, (response) => {
        if (chrome.runtime.lastError) {
